...
func (rt *_router) getOneMessage(w http.ResponseWriter, r *http.Request, ps httprouter.Params) { // dichiarazioe funzione base
	...
	parametroMessageId := ps.ByName("messageId")
	...
	*/
	...
	mess, err := strconv.Atoi(parametroMessageId) // conversione da stringa a int
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
	result, err := rt.db.GetOneMessage(mess) // chiamo la funzione
