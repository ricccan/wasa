...
func (rt *_router) getReactions(w http.ResponseWriter, r *http.Request, ps httprouter.Params) { // dichiarazioe funzione base
	...
	messaggioId := ps.ByName("messageId")

	messaggio, err := strconv.Atoi(messaggioId) // conversione da stringa a int
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}

	result, err := rt.db.GetReactions(messaggio) // chiamo la funzione
