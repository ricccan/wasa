...
func (rt *_router) commentMessage(w http.ResponseWriter, r *http.Request, ps httprouter.Params) { //dichiarazioe funzione base
	...
	idMessage, err := strconv.Atoi(parametroMessage) //conversione da stringa a int
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
	...
	var data commentMessageRequest
	...
	err = rt.db.CommentMessage(idUser, idChat, idMessage, imageData, data.Text)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
